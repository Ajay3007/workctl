 ...
 * File format:
 *   # Google — SDE2
 *   <!-- INTERVIEW_META: id=... company=... role=... date=... round=... result=... jobUrl=... created=... -->
 *
 *   ## Notes
 *   Overall impressions...
 *
...
public class InterviewService {
    ...
    public Interview createInterview(String company, String role, LocalDate date) {
        Interview interview = new Interview(company, role, date);
                ...
                stream
                    ...
                    .forEach(p -> {
                        try {
                            ...
                            if (iv != null) result.add(iv);
                        } catch (Exception ignored) {}
                    });
            }
            result.sort(Comparator.comparing(
                    iv -> iv.getDate() != null ? iv.getDate() : LocalDate.MIN,
    ...
    public boolean deleteInterview(String interviewId) {
        try {
            Path file = findInterviewFile(interviewId);
            if (file != null) { Files.deleteIfExists(file); return true; }
    ...
    private String formatInterview(Interview iv) {
        ...
        sb.append(" date=").append(iv.getDate() != null ? iv.getDate() : LocalDate.now());
        ...
        if (iv.getJobUrl() != null) sb.append(" jobUrl=").append(iv.getJobUrl());
    ...
    private Interview parseInterview(List<String> lines) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            if (line.startsWith("# ") && iv == null) {
                iv = new Interview("", "", LocalDate.now());
                continue;
            }
            ...
            if (line.startsWith("## ")) {
                ...
                if (heading.equals("Notes")) {
                    ...
                    while (i + 1 < lines.size() && !lines.get(i + 1).startsWith("#")) {
                        i++;
                        if (!lines.get(i).isBlank()) notes.append(lines.get(i)).append("\n");
                    }
            ...
            if (inExperienceLinks && line.trim().startsWith("- [")) {
                ...
                if (link != null) iv.getExperienceLinks().add(link);
                continue;
            }
            ...
            if (inQuestions && currentSection != null && line.trim().startsWith("- [")) {
                ...
                if (lastQuestion != null) iv.getQuestions().add(lastQuestion);
                continue;
            }
    ...
    private void parseMetaLine(String line, Interview iv) {
        ...
        Matcher dateM    = Pattern.compile("date=(\\d{4}-\\d{2}-\\d{2})").matcher(line);
        ...
        if (idM.find())      iv.setId(idM.group(1));
        if (companyM.find()) iv.setCompany(deslug(companyM.group(1)));
        if (roleM.find())    iv.setRole(deslug(roleM.group(1)));
        if (dateM.find())    { try { iv.setDate(LocalDate.parse(dateM.group(1))); } catch (Exception ignored) {} }
        if (roundM.find())   { try { iv.setRound(InterviewRound.valueOf(roundM.group(1))); } catch (Exception ignored) {} }
        if (resultM.find())  { try { iv.setResult(InterviewResult.valueOf(resultM.group(1))); } catch (Exception ignored) {} }
        if (jobUrlM.find()) {
            String url = jobUrlM.group(1);
            if (!url.startsWith("created=")) iv.setJobUrl(url);
        }
        if (creatM.find())   { try { iv.setCreatedAt(LocalDate.parse(creatM.group(1))); } catch (Exception ignored) {} }
    }
    ...
    private Path buildInterviewFilePath(Interview iv) throws IOException {
        String date    = iv.getDate() != null ? iv.getDate().toString() : LocalDate.now().toString();
